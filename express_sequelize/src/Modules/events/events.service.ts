...
export class EventsService {
  ...
  async getEventsWithWorkshops() {
    ...
    const workshopItems = await Workshop.findAll({
      where: {
    ...
    for (let i in events) {
      sorted.push({
        ...events[i],
        workshops: workshopItems.filter(workshopItem => workshopItem.eventId === events[i].id)
      });
    }
    return sorted;
  }
  ...
  async getFutureEventWithWorkshops() {
    ...
    const events = await Event.findAll({
      where: {
        id: workshopItems.map(item => item.eventId)
      },
      raw: true
    });
    const sorted = [];
